...
async function testFeature33() {
  ...
  try {
    ...
    const db = new Database(
      join(__dirname, "..", "server", "data", "noslimites.db"),
    );

    const timestamp = Date.now();
    const notifications = [
      {
        id: `notif-1-${timestamp}`,
        type: "relation_request",
        title: "Nouvelle demande",
        message: "User 1 vous a envoyé une invitation",
      },
      {
        id: `notif-2-${timestamp}`,
        type: "relation_accepted",
        title: "Invitation acceptée",
        message: "User 2 a accepté votre invitation",
      },
      {
        id: `notif-3-${timestamp}`,
        type: "new_common_limit",
        title: "Nouvelle limite commune",
        message: "Une nouvelle limite commune a été découverte",
      },
      {
        id: `notif-4-${timestamp}`,
        type: "limit_removed",
        title: "Limite retirée",
        message: "Une limite commune a été retirée",
      },
      {
        id: `notif-5-${timestamp}`,
        type: "relation_deleted",
        title: "Relation supprimée",
        message: "Une relation a été supprimée",
      },
    ];
