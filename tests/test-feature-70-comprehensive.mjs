...
async function runComprehensiveChecks() {
  ...
  try {
    const stdout = readFileSync(cssPath, "utf-8");

    const requiredMobileQueries = [
    ...
    if (!hasAny) {
      console.log(
        "✗ No explicit small-screen media queries found (recommended)",
      );
    } else {
    ...
    const categoryFlex =
      stdout.includes(".categoriesList") && stdout.includes("display: flex");
