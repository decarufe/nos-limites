...
async function verifySchema() {
  ...
  const tableNames = tablesResult.rows.map((t) => t.name as string);
  ...
  for (const [tableName, expectedColumns] of Object.entries(expectedTables)) {
    ...
    const columnNames = columnsResult.rows.map((c) => c.name as string);
    ...
    if (missingColumns.length === 0) {
      console.log(
        `✅ ${tableName}: All ${expectedColumns.length} columns present (${columnNames.join(", ")})`,
      );
    } else {
      console.log(
    ...
    if (extraColumns.length > 0) {
      console.log(
        `   ℹ️  Extra columns in ${tableName}: ${extraColumns.join(", ")}`,
      );
    }
  }
